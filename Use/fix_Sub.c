...
/**
 * @brief Performs a 32-bit hardware operation using the Arduino ALU.
 * This function iterates through 32 bits of the operands, sends them to the Arduino,
 * reconstructs the final 32-bit result, and then sign-extends it to a 64-bit long long.
 * @return The final calculated result as a correctly signed long long.
 */
long long executeAluOperation(long long op1, long long op2, const char *muxCode, int subAddFlag, bool *final_carry)
{
    // ใช้ unsigned long long สำหรับการคำนวณระดับบิตเพื่อหลีกเลี่ยงพฤติกรรมไม่คาดคิด
    unsigned long long result_raw = 0;
    int carry_in = (subAddFlag == 1) ? 1 : 0;

    printf("      [DEBUG] ALU(32-bit) Input: op1=%lld, op2=%lld, sub=%d\n", op1, op2, subAddFlag);

    // วนลูป 32 รอบสำหรับ 32-bit
    for (int i = 0; i < NUM_BITS; i++)
    {
        ...
        if (alu_result_bit)
        {
            // สร้างผลลัพธ์ดิบแบบ 32-bit
            result_raw |= (1ULL << i);
        }
        carry_in = alu_carry_out;
    }

    // Carry Flag คือ Carry-out ของบิตสุดท้าย (bit 31)
    *final_carry = (carry_in == 1);

    printf("      [DEBUG] ALU Raw 32-bit Result: 0x%08llX\n", result_raw);

    // --- ส่วนสำคัญ: การแปลงผลลัพธ์ 32-bit เป็น long long 64-bit (Sign Extension) ---
    long long final_result;

    // ตรวจสอบ Sign Bit (bit ที่ 31) ของผลลัพธ์ 32-bit
    if (result_raw & (1ULL << (NUM_BITS - 1)))
    {
        // ถ้าบิตที่ 31 เป็น 1 (ผลลัพธ์เป็นลบในระบบ 32-bit)
        // ให้เติม 1 ไปใน 32 บิตที่สูงกว่าเพื่อให้เป็นค่าลบที่ถูกต้องในระบบ 64-bit
        final_result = (long long)(result_raw | 0xFFFFFFFF00000000ULL);
    }
    else
    {
        // ถ้าบิตที่ 31 เป็น 0 (ผลลัพธ์เป็นบวก)
        // ค่าที่ได้ก็คือค่าเดิม (เนื่องจากบิตสูงกว่าของ result_raw เป็น 0 อยู่แล้ว)
        ...
        else if (strcmp(current.instruction, "SUB") == 0)
{
    ...
    // เรียกใช้ฟังก์ชัน ALU 32-bit
    // CARRY_FLAG จะถูกตั้งค่าภายในฟังก์ชันนี้
    long long result = executeAluOperation(val1, val2, "001", 1, &CARRY_FLAG);

    // result คือค่าที่ sign-extended มาเรียบร้อยแล้ว (เช่น -21)
    setRegisterValue(current.operand1, result);

    // ตอนนี้เราสามารถเช็คค่า result ได้โดยตรง เพราะมันเป็น signed long long ที่ถูกต้อง
    ZERO_FLAG = (result == 0);
    SIGN_FLAG = (result < 0);

    // CARRY_FLAG สำหรับ SUB: 1 = no borrow (A>=B), 0 = borrow (A<B)
