...
void alu_process_instruction(const char *instruction)
{
    ...
    {
        char result[128];
        alu_sub_binary(getVar(arg1), getVar(arg2), result, sizeof(result));
        setVar(arg1, result);
        printf("%s = %s\n", arg1, result);
    }
    ...
    {
        char result[128];
        alu_mul_binary(getVar(arg1), getVar(arg2), result, sizeof(result));
        setVar(arg1, result);
        printf("%s = %s\n", arg1, result);
    }
    ...
    {
        char result[128];
        alu_div_binary(getVar(arg1), getVar(arg2), result, sizeof(result));
        setVar(arg1, result);
        printf("%s = %s\n", arg1, result);
    }else {
...
static int alu_execute_bit(const char *muxCode, int subAdd, int aBit, int bBit)
{
    ...
    if (result < 0)
    {
        fprintf(stderr, "[ERROR] ส่งหรือรับข้อมูลล้มเหลว\n");
        return 0;
    }
    return result;
}
...
{
    char name[32];
    char value[64]; // เก็บเป็น string ของเลขฐานสอง
} Variable;
...
void setVar(const char *name, const char *value)
{
    ...
    strncpy(vars[varCount].name, name, sizeof(vars[varCount].name));
    strncpy(vars[varCount].value, value, sizeof(vars[varCount].value));
    varCount++;
}
